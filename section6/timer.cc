...
// When zero, we should defer timer interrupts and not call timer_handler
std::atomic<int> enabled_flag = 1;

// Non-zero when a timer event was deferred because enabled_flag was zero
...
void
timer_init(std::uint64_t usec, std::function<void()> handler)
{
    if (handler && usec) {
