...
std::string VMRegion::log;

VMRegion::Mapping::Mapping(VPage va)
    : va_(va), pi_({nullptr, PROT_NONE}), r_(VMRegion::find(va))
{
    assert(r_ != nullptr);
...
VMRegion::~VMRegion()
{
    ...
    if (munmap(base_, nbytes_) == -1)
        threrror("mmap");
#ifndef NDEBUG
    if (check_mapped_pages_on_exit && pages_mapped_ != 0) {
        std::cerr << "VMRegion destroyed with " << pages_mapped_
                << " pages still mapped" << std::endl;
        std::abort();
    }
#endif // !NDEBUG
}
...
VMRegion::map(VPage va, PPage pa, Prot prot)
{
    Mapping *m;
#ifndef NDEBUG
    if ((va < base_) || (va > (base_ + nbytes_))) {
        std::cerr << "VPage passed to VMRegion::map isn't in the region"
                << std::endl;
    }
    if (pa == nullptr) {
        std::cerr << "Null PPage passed to VMRegion::map" << std::endl;
        std::abort();
    }
#endif
    ...
    if (vm_id != -1) {
        log_printf("VMRegion %d page %d mapped with protection %s\n", vm_id,
            m->r_->page_num(va), prot_to_s(prot));
    } else {
        log_printf("VMRegion page %d mapped with protection %s\n",
            m->r_->page_num(va), prot_to_s(prot));
    }
    update(m, {pa, prot});
}
...
VMRegion::unmap(VPage va)
{
...
#ifndef NDEBUG
    if ((va < base_) || (va > (base_ + nbytes_))) {
        std::cerr << "VPage passed to VMRegion::unmap isn't in the region"
                << std::endl;
        std::abort();
    }
#endif
    ...
    if (it != pagemap_.end()) {
        if (vm_id != -1) {
            log_printf("VMRegion %d page %d unmapped\n", vm_id,
                it->second->r_->page_num(va));
        } else {
            log_printf("VMRegion page %d unmapped\n",
                it->second->r_->page_num(va));
        }
...
VMRegion::update(Mapping *m, PageInfo pi)
{
    ...
    if (pi.pa == nullptr) {
        ...
        if(mmap(m->va_, page_size, pi.prot, MAP_SHARED|MAP_FIXED,
                pm->fd_, pi.pa - pm->base_) == MAP_FAILED)
            threrror("mmap");
        ++*refcount(pi.pa);
        if (m->pi_.pa)
            --*refcount(m->pi_.pa);
...
VMRegion::fault_handler(int sig, siginfo_t *info, void *ctx)
{
    ...
    if (!r) {
        std::cerr << "page fault at invalid address 0x" << std::hex
                << std::uintptr_t(addr) << std::endl;
        std::abort();
    }
    try {
        if (r->vm_id != -1) {
            log_printf("Fault in VMRegion %d page %d\n", r->vm_id,
                        r->page_num(addr));
        } else {
            log_printf("Fault in VMRegion page %d\n", r->page_num(addr));
        }
        r->handler_(addr);
    }
...
void VMRegion::log_printf(const char* format, ...)
{
    std::string result;
	va_list ap;
	va_start(ap, format);
...
VMRegion::refcount(PPage pa)
{
    ...
    if (*c < 0) {
        std::cerr << "tried to use PPage that was already freed "
                  << "or never allocated."
                  << std::endl;
    }
    assert(*c >= 0);
...
PhysMem::~PhysMem()
{
    pools().erase(base_);
#ifndef NDEBUG
    if (check_used_pages_on_exit && nfree_ != npages_) {
        std::cerr << "PhysMem destroyed with " << npages_ - nfree_
                << " pages still in use" << std::endl;
        std::abort();
    }
#endif
    if (check_used_pages_on_exit) assert(nfree_ == npages_);
    munmap(base_, size_);
}
...
PhysMem::page_free(PPage p)
{
    ...
    if (*c != 0) {
        std::cerr << "page_free param already free or still mapped to VPage"
                  << std::endl;
    }
    assert(*c == 0);
    *c = -1;
