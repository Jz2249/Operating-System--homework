...
public:
    ...
    // Used to log various internal events (for testing).
    static std::string log;

    // Used to disable destructor error checking (for testing).
    bool check_mapped_pages_on_exit = true;
...
private:
    ...
    struct Mapping {
        ...
        PageInfo pi_;
        VMRegion *r_;          // region containing va_
    ...
    // Append message to log; printf--like arguments
    static void log_printf(const char* format, ...)
            __attribute__((format(printf, 1, 2)));
    
    // Return index of page in the region (for testing).
    int page_num(VPage va) {
        return (va - base_)/get_page_size();
    }
    
    // Return human-readable string corresponding to a Prot.
    static const char *prot_to_s(Prot prot) {
        if (prot == PROT_NONE) {
            return "none";
...
public:
    ...
    // Used to disable destructor error checking (for testing).
    bool check_used_pages_on_exit = true;
...
private:
    ...
    static std::map<PPage, PhysMem*> &pools() {
        ...
        static std::map<PPage, PhysMem*> &val =
                *new std::map<PPage, PhysMem*>;
        return val;
    }
