...
/**
 * Reads the specified sector (e.g. sectorNum) from the disk and stores
 * it at buf.  Returns the number of bytes read, or -1 on error.
 */
int diskimg_readsector(int fd, int sectorNum, void *buf); 

/**
 * Writes the information at buf to the specified sector on disk.
